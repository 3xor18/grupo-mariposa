...
| Tasas de impuesto | valores fijos por mercado y categoría en `config-repo` | colección `tax_rates` con `validFrom`/`validTo`, tasa elegida por `occurredAt`, `taxRateEffectiveFrom` en el pedido, API `/tax-rates` con aprobación de cuatro ojos y `config-repo` como semilla y respaldo ([ADR 0008](adr/0008-effective-dated-tax-rates.md)) | los cambios de IVA llegan con fecha y deben aplicarse sin despliegue y con auditoría |
...
| Tabla de tasas refrescada por intervalo en cada pod | sin tópico ni consumidor nuevos; `validFrom` debe ser futuro, así que basta aprobar con más anticipación que el intervalo (30 s) | evento `pricing.tax-rates.changed.v1` y anticipación mínima en la API (TODO-6, ADR 0008) |
| `orders_amount_total` suma montos aprobados en la moneda de cada mercado | KPI de demostración para el tablero `mariposa-demo`, no un dato contable | reporte financiero desde la base de pedidos, con conversión de moneda y cortes por fecha |
