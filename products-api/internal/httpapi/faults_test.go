...
func faultHarness(t *testing.T, raw string, hold time.Duration) harness {
	t.Helper()
	rules, err := fault.ParseRules(raw)
	if err != nil {
		t.Fatalf("rules: %v", err)
	}
	return newHarness(t, func(d *httpapi.Dependencies) {
...
func TestFaultInjectionStatuses(t *testing.T) {
	h := faultHarness(t, "PRD-001:400,PRD-002:429,PRD-003:500,PRD-004:502,PRD-008:503", time.Second)
	cases := []struct {
		target string
		status int
		code   httpapi.Code
	...
	for _, tc := range cases {
		t.Run(tc.target, func(t *testing.T) {
			...
			wantRetry := tc.status == http.StatusTooManyRequests
			if (rec.Header().Get("Retry-After") == "1") != wantRetry {
...
func TestFaultInjectionRecoversAfterTimes(t *testing.T) {
	h := faultHarness(t, "PRD-012:503:2", time.Second)
...
func TestFaultTimeoutHoldsResponse(t *testing.T) {
	const hold = 30 * time.Millisecond
	h := faultHarness(t, "PRD-013:timeout", hold)
	started := time.Now()
...
func TestFaultTimeoutStopsWhenClientCancels(t *testing.T) {
	h := faultHarness(t, "PRD-013:timeout", time.Minute)
	...
	select {
	case rec := <-done:
		assertProblem(t, rec, http.StatusServiceUnavailable, httpapi.CodeServiceUnavailable)
