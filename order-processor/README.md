...
| `KAFKA_MAX_POLL_INTERVAL_MS`, `KAFKA_SESSION_TIMEOUT_MS`, `KAFKA_DELIVERY_TIMEOUT_MS`, `KAFKA_REQUEST_TIMEOUT_MS`, `KAFKA_LINGER_MS` | `300000`, `45000`, `120000`, `30000`, `5` | clientes Kafka |
| `KAFKA_CREATE_TOPICS` / `KAFKA_REPLICATION_FACTOR` | `true` / `1` | creación de tópicos |
| `RECORD_RETRY_INITIAL_INTERVAL` / `RECORD_RETRY_MULTIPLIER` / `RECORD_RETRY_MAX_ATTEMPTS` | `1s` / `2.0` / `4` | reintento a nivel registro |
| `KAFKA_DLT_RETRY_DELAY` | `1s` | pausa si la DLT no está disponible |
...
| `PROCESSING_MAX_CONCURRENT_LOOKUPS` | `64` | fan-out máximo por instancia |
...
| `OUTBOX_RELAY_ENABLED` / `OUTBOX_RELAY_INTERVAL` / `OUTBOX_BATCH_SIZE` / `OUTBOX_LEASE` / `OUTBOX_SEND_TIMEOUT` | `true` / `250ms` / `100` / `30s` / `10s` | relay |
| `OUTBOX_RETRY_INITIAL_BACKOFF` / `OUTBOX_RETRY_MAX_BACKOFF` | `1s` / `60s` | reintento de publicación |
| `API_DEFAULT_PAGE_SIZE` / `API_MAX_PAGE_SIZE` | `20` / `100` | paginación de `GET /orders` |
| `AUTH_ENABLED`, `AUTH_ISSUER`, `AUTH_JWKS_URL`, `AUTH_READER_ROLE`, `AUTH_ADMIN_ROLE` | `true`, realm `mariposa` local, `orders-reader`, `orders-admin` | JWT de Keycloak |
