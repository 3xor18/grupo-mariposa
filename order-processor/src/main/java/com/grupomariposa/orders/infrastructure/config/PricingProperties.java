...
public record PricingProperties(
        @NotEmpty Map<String, @NotEmpty Map<TaxCategory, @NotNull @DecimalMin(DECIMAL_ZERO)
    ...
    private static final String UNCOVERED_MARKET =
            "Every catalog market needs STANDARD, REDUCED and EXEMPT rates in pricing.tax-rates";
    ...
    public TaxRateTable taxRateTable(final MarketCatalog catalog) {
        final Map<MarketCode, Map<TaxCategory, Rate>> table = new LinkedHashMap<>();
        taxRates.forEach((market, categories) -> {
            ...
            table.put(new MarketCode(market), rates);
        });
        final TaxRateTable rates = new TaxRateTable(table);
        if (!rates.covers(catalog.supportedMarkets())) {
            throw new IllegalStateException(UNCOVERED_MARKET);
        }
        return rates;
    }
